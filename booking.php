                ...
                <tr>
                   <td colspan="4">
                       ...
                       <div style="font-size: 18px; color: red;">
                        <br><br>
                        <?php
                            // Checking if the session is fully booked
                            if (isset($current_appointments) && $current_appointments < $appointment_limit) {
                                echo "Please arrive 10 minutes early for your appointment.";
                            }
                        ?>
                        ...
                        <table width="100%" class="sub-table scrolldown" border="0" style="padding: 50px;border:none">
                            ...
                            if ($_GET) {
                                if (isset($_GET["id"])) {
                                    ...
                                    // Check the number of existing appointments
                                    $sql2 = "SELECT * FROM appointment WHERE scheduleid = $id";
                                    $result12 = $database->query($sql2);
                                    $current_appointments = $result12->num_rows;

                                    // Set the appointment limit
                                    $appointment_limit = 10;  // Adjust as needed

                                    // Check if the session is fully booked
                                    if ($current_appointments >= $appointment_limit) {
                                        // Display a message if the limit is reached
                                        echo "<div style='font-size: 18px; color: red;'>Sorry, this session is fully booked. Please choose another session.</div>";
                                    } else {
                                        // Display the booking form if the limit is not reached
